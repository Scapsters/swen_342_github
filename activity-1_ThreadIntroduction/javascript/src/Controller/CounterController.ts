...
{
    static readonly Holder: WorkerHolder<CounterController> = new WorkerHolder()
    static stopWorkerOnSelectedWorker() { CounterController.Holder.get(CounterView.getSelectedWorkerId())?.destructor() }
    static startCountOnSelectedWorker() { CounterController.Holder.get(CounterView.getSelectedWorkerId())?.startCount() }
    static stopCountOnSelectedWorker()  { CounterController.Holder.get(CounterView.getSelectedWorkerId())?.stopCount() }
    static resetCountOnSelectedWorker() { CounterController.Holder.get(CounterView.getSelectedWorkerId())?.resetCount() }
    ...
    startCount() { this.worker.postMessage({ message: 'start count' }) }
    stopCount()  { this.worker.postMessage({ message: 'end count' }) }
    resetCount() { this.worker.postMessage({ message: 'reset count' })}
}
