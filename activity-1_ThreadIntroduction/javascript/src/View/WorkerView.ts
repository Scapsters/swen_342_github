...
export class WorkerView {
	...
	static readonly workerViewElement = nullCheck(
		document.getElementById("WorkerView"),
		"WorkerView"
	);
	...
	private createElement() {
		const threadTemplate = nullCheck(
			document.getElementById(this.templateId),
			`${this.templateId}`
		) as HTMLTemplateElement;
		const clone = nullCheck(
			threadTemplate.content.cloneNode(true),
			"clone"
		);
		...
		this.displayId = nullCheck(
			this.wrapper.querySelector(".id"),
			"displayId"
		);
		this.displayState = nullCheck(
			this.wrapper.querySelector(".state"),
			"displayState"
		);
	...
	removeElement() {
		nullCheck(
			nullCheck(this.wrapper, "node").parentNode,
			"parentNode"
